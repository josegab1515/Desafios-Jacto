...
from controllers.crud_mongo import (
    get_all_cars, get_car_by_id, post_cars, put_cars, delete_cars
)
from models.cars_schema import CarSchema, ValidationError

car_bp = Blueprint("car_bp", __name__)

@car_bp.route("/cars/mongo", methods=["GET"])
def get_all_cars_route():
    data = get_all_cars()
    return jsonify(data)


@car_bp.route("/cars/mongo/<car_id>", methods=["GET"])
...
@car_bp.route("/cars/mongo", methods=["POST"])
def post_cars_route():
    data = request.json
...
@car_bp.route("/cars/mongo/<car_id>", methods=["PUT"])
...
@car_bp.route("/cars/mongo/<car_id>", methods=["DELETE"])
